...
export type ArtistWithMembers = Prettify<
  Artist & {
    members: Member[]
  }
>

export type ArtistWithContent = Prettify<
  Artist & {
    company: Company
    members: Member[]
    albums: Album[]
...
export async function fetchArtistsWithMembers(
  ...
  const result = rows.reduce<{ artist: Artist; members: Member[] }[]>((acc, row) => {
...
export async function fetchArtistWithMembers(
  artistId: number | string,
): Promise<ArtistWithMembers | undefined> {
  const result = await fetchArtistsWithMembers(artistId)
  return result[0]
}

export async function fetchArtistsWithContent(
  artistId?: number | string,
): Promise<ArtistWithContent[]> {
  if (Number.isNaN(Number(artistId))) {
    return []
  }
  ...
  const result = rows.reduce<
    { artist: Artist; members: Member[]; company: Company; albums: Album[] }[]
  >((acc, row) => {
    ...
    const memberExists =
      acc[artist.id]?.members &&
      acc[artist.id]?.members.findIndex((a) => a.id === member?.id) !== -1
    if (member && !memberExists) {
      acc[artist.id]?.members.push(member)
    }

    const albumExists =
      acc[artist.id]?.albums && acc[artist.id]?.albums.findIndex((a) => a.id === album?.id) !== -1
...
export async function fetchArtistWithContent(
  artistId: number | string,
): Promise<ArtistWithContent | undefined> {
