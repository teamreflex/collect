...
export default function AlbumIcon({ album, size = 'default' }: Props) {
  return (
    <Link aria-label={album.name} href={{ pathname: `/albums/${album.id}` }} className={cn("relative overflow-hidden group shadow-md", size === 'default' ? 'rounded-2xl w-64 h-64' : 'rounded-lg w-32 h-32')}>
