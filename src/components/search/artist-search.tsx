...
export function ArtistSearch({ onSelected, value }: Props) {
  ...
  const { isFetching } = api.artists.search.useQuery(debouncedSearch, {
  ...
  const { isFetching: isFetchingInitialValue } = api.artists.fetch.useQuery(value as string, {
    ...
    <div className={cn("flex w-full flex-row", selected ? "gap-2" : null)}>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {!value && <span>Search for an artist...</span>}
            {value && isFetchingInitialValue && <Loader2 className="h-4 w-4 animate-spin" />}
            {value && selected && (
              <span>{options.find((opt) => String(opt.id) === String(value))?.nameEn}</span>
            )}
        ...
        <PopoverContent className="p-0">
          <Command shouldFilter={false}>
            <CommandInput onValueChange={setSearchTerm} placeholder="Search artists..." />
            {isFetching && (
