...
export default function UpdatePhotocardSet({ album, photocardSet }: Props) {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<UpdatePhotocardSetSchema>({
    resolver: zodResolver(updatePhotocardSetSchema),
    defaultValues: {
      ...photocardSet,
  ...
  const { mutate: updatePhotocardSet, isLoading } = api.photocardSets.update.useMutation({
    onSuccess(_, newData) {
      router.refresh()
      setOpen(false)
      reset(newData)
      toast({
        description: (
          <p>
            Photocard set <span className="font-semibold">{newData.name}</span> updated
          </p>
        ),
      })
    },
  ...
  function onSubmit(data: UpdatePhotocardSetSchema) {
    updatePhotocardSet(data)
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>Update Photocard Set</DialogTitle>
            <DialogDescription>Update an existing photocard set.</DialogDescription>
          </DialogHeader>
