  ...
  return (
    ...
    >
      ...
      <div className="relative z-50 flex h-full flex-col items-center justify-center gap-2 p-2">
        <Image alt={album.name} src={album.image} height={100} width={100} className="rounded-xl shadow" />
        <p className="text-center text-xl font-semibold drop-shadow text-white">{album.name}</p>
      </div>
    </Link>
  )
}
